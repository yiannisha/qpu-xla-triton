...
@pytest.mark.hardware
@pytest.mark.skipif(not Path("/dev/dri/renderD128").exists(), reason="VideoCore VII unavailable")
def test_fused_gemma_flash_attention_mx_matches_oracle() -> None:
    rng = np.random.default_rng(4963)
    rows, heads, kv_rows, head_dim = 3, 8, 32, 256
    query_host = rng.normal(0.0, 0.15, size=(rows, heads, head_dim)).astype(np.float32)
    key_host = rng.normal(0.0, 0.15, size=(kv_rows, head_dim)).astype(np.float16)
    value_host = rng.normal(0.0, 0.2, size=(kv_rows, head_dim)).astype(np.float16)
    mask_host = np.zeros((rows, kv_rows), dtype=np.float16)
    mask_host[1, -5:] = -np.inf
    mask_host[2, -9:] = -np.inf
    scale = float(head_dim**-0.5)
    expected = ggml_flash_attn_ext_reference(
