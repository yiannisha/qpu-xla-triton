...
int main(void) {
    uint8_t weights[OUTPUT_COLUMNS][Q6_K_BYTES] = {{0}};
    ...
    for (unsigned int column = 0; column < OUTPUT_COLUMNS; ++column) {
        const int8_t code = (int8_t) ((int) (column % 64U) - 32);
        const uint8_t encoded = (uint8_t) ((int) code + 32);
        const uint8_t low = encoded & UINT8_C(0x0f);
        const uint8_t high = encoded >> 4U;
        memset(weights[column], low | (uint8_t) (low << 4U), 128);
        memset(weights[column] + 128, (uint8_t) (high * UINT8_C(0x55)), 64);
        memset(weights[column] + 192, 1, 16);
        memcpy(weights[column] + 208, &one_fp16, sizeof(one_fp16));
    }
    ...
    qpu_llama_q6_k_linear *linear = NULL;
    status = qpu_llama_q6_k_linear_prepare(context, &desc, &linear);
    if (status != QPU_LLAMA_OK) {
    ...
    status = qpu_llama_q6_k_linear_execute(linear, &execution, &timing);
    int result = 0;
    if (status != QPU_LLAMA_OK) {
    ...
    for (unsigned int row = 0; row < ROWS; ++row) {
        ...
        for (unsigned int column = RESIDENT_START; column < OUTPUT_COLUMNS; ++column) {
            const int code = (int) (column % 64U) - 32;
            const float expected = (float) (256 * (int) (row + 1U) * code);
