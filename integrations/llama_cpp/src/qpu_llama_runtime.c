...
qpu_llama_status qpu_llama_program_execute(
    ...
    pthread_mutex_lock(&context->mutex);
    if (context->poisoned) {
        set_error(context, "V3D context is poisoned after a wait failure");
        pthread_mutex_unlock(&context->mutex);
        return QPU_LLAMA_IO_FAILED;
    }
    ...
    append_handle(handles, &handle_count, program->code->handle);
    append_handle(handles, &handle_count, program->uniforms->handle);
    for (uint32_t index = 0; index < dispatch->buffer_count; ++index) {
        ...
        append_handle(handles, &handle_count, dispatch->buffers[index]->handle);
    }
    ...
    for (uint32_t index = 0; index < handle_count; ++index) {
        struct drm_v3d_wait_bo wait = {.handle = handles[index], .pad = 0, .timeout_ns = context->timeout_ns};
