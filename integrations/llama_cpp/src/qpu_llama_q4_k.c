...
qpu_llama_status qpu_llama_q4_k_linear_prepare(
    qpu_llama_context *context,
    const qpu_llama_q4_k_linear_desc *desc,
    qpu_llama_q4_k_linear **result) {
    ...
    *result = NULL;
    ...
    if (!multiply_size(blocks, Q4_K_BLOCK_BYTES, &weight_row_bytes) ||
    ...
    qpu_llama_q4_k_linear *linear = calloc(1, sizeof(*linear));
    if (linear == NULL) {
    ...
    if (status == QPU_LLAMA_OK) {
        ...
        const qpu_llama_program_desc program_desc = {
            .code = qpu_ggml_q4_k_q8_k_m4,
            .code_size = sizeof(qpu_ggml_q4_k_q8_k_m4),
            .compiled_source_hash = qpu_ggml_q4_k_q8_k_m4_source_hash,
            .expected_source_hash = desc->expected_source_hash,
            .binary_sha256 = qpu_ggml_q4_k_q8_k_m4_binary_hash,
            .uniform_word_count = 14,
        };
...
void qpu_llama_q4_k_linear_destroy(qpu_llama_q4_k_linear *linear) {
    if (linear == NULL) {
        return;
    }
...
qpu_llama_status qpu_llama_q4_k_linear_execute(
    qpu_llama_q4_k_linear *linear,
...
size_t qpu_llama_q4_k_linear_resident_bytes(const qpu_llama_q4_k_linear *linear) {
