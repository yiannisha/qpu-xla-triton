...
qpu_llama_status qpu_llama_q8_0_linear_prepare(
    ...
    if (context == NULL || desc == NULL || result == NULL || desc->weights == NULL ||
        ...
        desc->input_columns % QK8_0 != 0 || desc->output_columns == 0 ||
    ...
    *result = NULL;
    const size_t blocks = desc->input_columns / QK8_0;
    ...
    if (!multiply_size(blocks, Q8_0_BLOCK_BYTES, &weight_row_bytes) ||
        ...
        !multiply_size(blocks, Q8_0_BLOCK_BYTES, &activation_row_bytes) ||
        !multiply_size(ROWS, activation_row_bytes, &activation_bytes) ||
        desc->weight_size < required_source_bytes) {
        return QPU_LLAMA_INVALID_ARGUMENT;
    }
    qpu_llama_q8_0_linear *linear = calloc(1, sizeof(*linear));
    if (linear == NULL) {
    ...
    if (status == QPU_LLAMA_OK) {
        ...
        const qpu_llama_program_desc program_desc = {
            .code = qpu_ggml_q8_0_q8_0_m4,
            .code_size = sizeof(qpu_ggml_q8_0_q8_0_m4),
            .compiled_source_hash = qpu_ggml_q8_0_q8_0_m4_source_hash,
            .expected_source_hash = desc->expected_source_hash,
            .binary_sha256 = qpu_ggml_q8_0_q8_0_m4_binary_hash,
            .uniform_word_count = 10,
        };
